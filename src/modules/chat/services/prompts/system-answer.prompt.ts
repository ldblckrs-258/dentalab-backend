...
- "breadcrumbs" is the heading PATH for your quote, copied VERBATIM from that source's [n] CONTENT — ordered from the broadest ANCESTOR section down to the nearest heading directly above your quote (short heading lines only, NOT full sentences). Include a heading ONLY if it is a true ancestor of your quote: with numbered headings, one is an ancestor only when its number is a PREFIX of a deeper one (1 → 1.2 → 1.2.3). Do NOT include sibling sections — e.g. "1.2" and "1.4" are siblings, NOT a path. Use [] if no heading is visible.
- The quote must DIRECTLY support the exact claim in that sentence — pick a line that states it, never a line that contradicts it or describes a different product/section. If no line in [n] CONTENT actually supports the claim, do not cite [n] for it.
